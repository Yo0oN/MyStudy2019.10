...
<%
	...
	try {
		...
		String sql = "select seq, grpl, subject, writer, date_format(wdate, '%Y-%m-%d') wdate, hit, datediff(now(), wdate) diff from rep_board1 order by grp desc, grps asc";
		...
		for (int i = 0; i < recordPerPage && rs.next(); i++) {
			...
			String sgrpl = "";
			// grpl의 숫자만큼 공백을 넣어준다.(그룹에서의 깊이만큼 공백을 넣어준다.)
			// 만약 grpl이 0이면 제일 상단글, 모글이라는 뜻이다. 모글일경우 앞에 공백이없지만,
			// grpl이 1인 답글일 경우에는 모글보다는 들여써야 보기 편하기 때문에 앞에 띄어쓰기 두번 해준다.
			// grpl 수가 늘어나는만큼 들여쓰기양도 많아질것이다.
			...
			// 만약 grpl이 0이 아니라면, 즉 답글들일 경우에는 앞에 답글그림을 넣어주고, 띄어쓰기도 해준다.
			if (grpl != 0) {
				sbHtml.append(sgrpl + "<img src='../../images/re.gif' width='9' />");
			}
			sbHtml.append("<a href='board_view1.jsp?seq=" + seq + "&cpage=" + cpage + "'> " + subject);
			if (diff == 1) {
